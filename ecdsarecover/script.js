function init() {
    ...
    document.getElementById('sv_btn').addEventListener('click', doSignAndVerify);
}

/**
 * 署名＆検証開始
 */
function doSignAndVerify() {
    this.disabled = true;
    let ids = ['e', 'x', 'px', 'py', 'v', 'r', 's', 'rx', 'ry', 'ret'];
    for (let id of ids) {
        document.getElementById('sv_' + id).innerText = '';
    }
    setTimeout(signAndVerify, 10);
}
...
function signAndVerify() {
    let e = rnd32();
    console.log('e', hex32(e));
    document.getElementById('sv_e').innerText = '0x' + hex32(e);
    let x = rnd32();
    console.log('x', hex32(x));
    document.getElementById('sv_x').innerText = '0x' + hex32(x);
    let P = ECDSAPUBKEY(x);
    console.log('P.x', hex32(P.x));
    console.log('P.y', hex32(P.y));
    document.getElementById('sv_px').innerText = '0x' + hex32(P.x);
    document.getElementById('sv_py').innerText = '0x' + hex32(P.y);
    let sig = ECDSASIGN(e, x);
    document.getElementById('sv_v').innerHTML = sig.v;
    document.getElementById('sv_r').innerText = '0x' + hex32(sig.r);
    document.getElementById('sv_s').innerText = '0x' + hex32(sig.s);
    let R = ECDSARECOVER(e, sig.v, sig.r, sig.s);
    console.log('R.x', hex32(R.x));
    console.log('R.y', hex32(R.y));
    document.getElementById('sv_rx').innerText = '0x' + hex32(R.x);
    document.getElementById('sv_ry').innerText = '0x' + hex32(R.y);
    let ret = (P.x == R.x) && (P.y == R.y);
    console.log('Result', ret);
    if (ret) {
        document.getElementById('sv_ret').innerText = '正常（公開鍵一致）';
    } else {
        document.getElementById('sv_ret').innerText = '異常（公開鍵不一致）';
    }
    document.getElementById('sv_btn').disabled = false;
}
