...
function init() {
    document.getElementById("mnemonictest1btn").addEventListener("click", test1);
    document.getElementById("mnemonictest2btn").addEventListener("click", test2);
}

function test1() {
    document.getElementById("result").innerHTML = "";
    log("Mnemonic TEST1 BEGIN");
    let m = new Mnemonic();
    let total = 0;
    let ok = 0;
    for (let test of TEST_VECTOR_1["english"]) {
        let entropy = test[0];
        let mnemonic = test[1];
        let passphrase = "TREZOR";
        let seed = test[2];
        let xprv = test[3];
        if (check(m, entropy, mnemonic, passphrase, seed, xprv)) {
            log("OK " + mnemonic);
            ok++;
        }
        total++;
    ...
    if (ok == total) {
        ...
        log("TEST NG " + ok + " / " + total);
    }
    log("Mnemonic TEST1 END");
}

function test2() {
    document.getElementById("result").innerHTML = "";
    log("Mnemonic TEST2 BEGIN");
    var m = new Mnemonic("japanese");
    let total = 0;
    let ok = 0;
    for (let test of TEST_VECTOR_2) {
        let entropy = test.entropy;
        let mnemonic = test.mnemonic;
        let passphrase = test.passphrase;
        let seed = test.seed;
        let xprv = test.bip32_xprv;
        if (check(m, entropy, mnemonic, passphrase, seed, xprv)) {
            log("OK " + mnemonic);
            ok++;
        }
    ...
    log("Mnemonic TEST2 END");
}

var hdw = new HDW();

function check(m, entropy, mnemonic, passphrase, seed, xprv) {
    var mne = m.toMnemonic(entropy);
    if (mne != mnemonic) {
        log("NG mnemonic");
        log(mne);
        log(mnemonic);
